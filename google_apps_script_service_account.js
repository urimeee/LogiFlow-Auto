 ...
 * 4. 스크립트 속성에 WEBHOOK_SECRET 설정
 * 5. setupTrigger() 실행하여 onEdit 트리거 등록
 */
...
// 대상 시트 gid (선택사항 - 지정하지 않으면 모든 시트 감지)
const TARGET_GID = '1735735926';
...
function onEdit(e) {
  try {
    ...
    if (TARGET_GID) {
      ...
      if (sheetId !== TARGET_GID) {
        Logger.log(`Sheet gid ${sheetId} does not match target gid ${TARGET_GID}`);
        return;
      }
    }
...
function sendSheetDataToWebhook(sheet) {
  try {
    ...
    const token = scriptProperties.getProperty('WEBHOOK_SECRET');
    
    if (!token) {
      Logger.log('⚠️ WARNING: WEBHOOK_SECRET not set in Script Properties!');
      Logger.log('Go to Project Settings → Script Properties → Add WEBHOOK_SECRET');
    }
...
function testWebhook() {
  try {
    ...
    Logger.log(`Testing with sheet: ${sheet.getName()} (gid: ${sheet.getSheetId()})`);
...
function checkConfiguration() {
  ...
  Logger.log(`Target gid: ${TARGET_GID || 'All sheets'}`);
  
  const scriptProperties = PropertiesService.getScriptProperties();
  const token = scriptProperties.getProperty('WEBHOOK_SECRET');
  
  if (token) {
    Logger.log(`✅ WEBHOOK_SECRET is set (length: ${token.length})`);
  } else {
    Logger.log('❌ WEBHOOK_SECRET is NOT set!');
    Logger.log('To set it:');
    Logger.log('1. Click the gear icon (Project Settings)');
    Logger.log('2. Go to Script Properties');
    Logger.log('3. Add property: WEBHOOK_SECRET = your_secret_token');
  }
...
function previewData() {
  ...
  Logger.log(`Sheet: ${sheet.getName()} (gid: ${sheet.getSheetId()})`);
