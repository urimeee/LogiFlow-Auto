...
4. **"➕ 주문 추가"** 버튼 클릭
5. 추가된 주문 목록 확인
6. 파일 업로드 주문과 함께 통합 처리

#### 주문번호 자동 생성 규칙:
- **시딩**: `{오늘날짜}_Seed_{고유번호}` (예: `20260322_Seed_a`, `20260322_Seed_b`)
- **CS**: `{오늘날짜}_CS_{고유번호}` (예: `20260322_CS_a`, `20260322_CS_b`)
- 고유번호는 a부터 알파벳 순서로 자동 생성

#### 특징:
- ✅ 마스터 코드에서 상품 정보 자동 매칭
- ✅ 3PL 형식으로 자동 변환
- ✅ 파일 업로드 주문과 함께 통합 다운로드
- ✅ 실시간 주문 목록 확인
- ✅ 일괄 삭제 기능
...
✅ **수동 주문 입력 기능 추가 - 시딩/CS 주문 직접 입력**  
  - **신규 기능**:
    ...
    * 파일 업로드 주문과 함께 통합 처리
  - **기술 구현**:
    * `st.session_state`로 수동 주문 목록 관리
